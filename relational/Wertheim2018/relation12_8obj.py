...
class Relational12_8(pytry.NengoTrial):
    def params(self):
    ...
    def model(self, p):
        ...
        vocab_space.populate('OBJ1; OBJ2; OBJ3; OBJ4; OBJ5; OBJ6; OBJ7; OBJ8; X; Y')

        vocab_rule = spa.Vocabulary(p.D)
        vocab_rule.populate('OBJ1; OBJ2; OBJ3; OBJ4; OBJ5; OBJ6; OBJ7; OBJ8; BELOW; ABOVE; LEFT; RIGHT; S; O; V')

        vocab_obj = vocab_rule.create_subset(['OBJ1', 'OBJ2', 'OBJ3', 'OBJ4', 'OBJ5', 'OBJ6', 'OBJ7', 'OBJ8'])
        ...
        with model:
            ...
            def input(t):
                ...
                if t<0.4:
                    ...
                    return 'OBJ6*S+LEFT*V+OBJ7*O'
                elif t<2.8:
                    return 'OBJ7*S+LEFT*V+OBJ8*O'          
            ...
            display = nengo.Node(display_node, size_in=16)
            ...
            nengo.Connection(objs.output, display[8], 
                      transform=[vocab_space.parse('OBJ5*X').v])
            nengo.Connection(objs.output, display[9], 
                      transform=[vocab_space.parse('OBJ5*Y').v])              
            nengo.Connection(objs.output, display[10], 
                      transform=[vocab_space.parse('OBJ6*X').v])
            nengo.Connection(objs.output, display[11], 
                      transform=[vocab_space.parse('OBJ6*Y').v])
            nengo.Connection(objs.output, display[12], 
                      transform=[vocab_space.parse('OBJ7*X').v])
            nengo.Connection(objs.output, display[13], 
                      transform=[vocab_space.parse('OBJ7*Y').v])
            nengo.Connection(objs.output, display[14], 
                      transform=[vocab_space.parse('OBJ8*X').v])
            nengo.Connection(objs.output, display[15], 
                      transform=[vocab_space.parse('OBJ8*Y').v])
    ...
    def evaluate(self, p, sim, plt):
        ...
        final_data = data[-1]
        obj1x, obj1y, obj2x, obj2y, obj3x, obj3y, obj4x, obj4y, obj5x, obj5y, obj6x, obj6y, obj7x, obj7y, obj8x, obj8y = final_data
        if obj1x<obj2x<obj3x<obj4x<obj5x<obj6x<obj7x<obj8x:
            score = 1
        else:
            score = 0
        ...
        if plt:
            ...
            plt.legend(loc='best', labels=['1','2','3','4','5','6','7','8'])
            plt.ylabel('x value')
            plt.subplot(2,1,2)
