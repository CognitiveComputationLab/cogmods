...
class Queue():
    ...
    def find_reference(self, relation):
        """ Find the reference item for an item to be inserted.
    ...
    def remove_first_obj(self):
        self.first = self.first.next
        
    def remove_obj(self, obj1, obj2):
        """ Removes first foudn object out of queue and returns it

        Parameters
        ----------
        obj1, obj2 : str
            object as string to remove from queue
        """
        current = self.first
        before = None
        while True:
            if current.data == obj1 or current.data == obj2:
                if before == None:
    ...
    def include_counterfact(self, relation):
        """ Include fact that isn't in alignment with the queue into the queue.
        ...
        relation : list
            ...
            reference object strings, of the relation.
        """
        firstFoundObj = self.remove_obj(relation[1], relation[2])
        self.insert_last(self.first, firstFoundObj)
    ...
    def get_model(self):
        """
        returns model of form "ABC" or "BAC" etc.
        """
        model = ""
        current = self.first
        direction = self.origin.lower()
        if direction == "right":
            while current is not None:
...
class VerbalModel(ccobra.CCobraModel):
    ...
    def __init__(self, name='Verbal'):
        ...
        # saves model constructed in sequence 1
        self.allModels = {}
    ...
    def construct_queue(self, task, force_origin=0):
        ...
        origin = self.decide_origin(task[1], dimension, force_origin)
        ...
        while len(queues) > 1:
            queues = self.fuse_all_queues(queues)
        preferred_origin = "left" if self.preferences['left'] >= self.preferences['right'] else "right"
        if queues[0].origin.lower() != preferred_origin:
            queues[0].origin = preferred_origin
            queues[0].reverse()
        return queues[0]
    ...
    def compete(self, item, response, event=""):
            ...
            if len(valid_reverse) > 0:
                if random.choice(valid_reverse) == response[0]:
                    for i in q_reverse.origin.lower().split("-"):
                        self.preferences[i + "-reverse"] += 1
        if event != "":
            q_basic = self.construct_queue(item.task)
            self.allModels[event] = q_basic
    ...
    def pre_train(self, dataset):
        ...
        for person_data in dataset:
            for item_data in person_data:
                if item_data["item"].sequence_number != 5:
                    continue
                item = item_data["item"]
    ...
    def predict(self, item, **kwargs):
        ...
        """
        event = kwargs['event'][:35]
        if item.response_type == "verify":
            q = self.allModels[event]
            return q.validate(item.choices[0][0])
        if item.response_type == "single-choice":
            if item.sequence_number == 1:
                q = self.construct_queue(item.task)
                self.allModels[event] = q
                return q.get_model()
            elif item.sequence_number == 3:
                counterfact = item.task[1]
                q = self.allModels[event]
                q.include_counterfact(counterfact)
                model = q.get_model()
                return model
    ...
    def adapt(self, item, response, **kwargs):
        ...
        taskRel = item.task[1][0].lower()
        if taskRel == "left":
            # Left object was relocated, therefore origin is left
            if initialModel[0] == response[2]:
                self.preferences['left'] += 1
            # Right object was relocated, therefore origin is right
            elif initialModel[2] == response[0]:
                self.preferences['right'] += 1
        elif taskRel == "right":
            # Left object was relocated, therefore origin is left
            if initialModel[0] == response[2]:
                self.preferences['left'] += 1
                self.preferences['left-reverse'] += 1
            # Right object was relocated, therefore origin is right
            elif initialModel[2] == response[0]:
                self.preferences['right-reverse'] += 1
                self.preferences['right'] += 1
