...
class RNN(nn.Module):
    def __init__(self, input_size=8, hidden_size=32, output_size=8):
        ...
        self.h2o = nn.Linear(hidden_size, 8)
...
class LSTMModel(ccobra.CCobraModel):
    def __init__(self, name='LSTM', k=1):
        super(LSTMModel, self).__init__(name, ["spatial-relational"], ["single-choice"])
    ...
    def train_network(self, train_x, train_y, n_epochs, verbose=False):
        ...
        if verbose:
            ...
            for subj_idx in range(len(self.train_x)):
                pred, _ = self.net(self.train_x[subj_idx].view(-1,1,8), None)
