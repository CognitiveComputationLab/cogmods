...
class VerbalReasoning_ldt(ccobra.CCobraModel):
    """ This model uses the results of the ldt and the response time of the participants
    in order to decide when to reencode their verbal model
    """
    def __init__(self, name='Verbal Reasoning adaptiv + selective processing'):
        super(VerbalReasoning_ldt, self).__init__(
    ...
    def predict(self, item, **kwargs):
        ...
        if kwargs["ldt_uw"] - kwargs["ldt_rw"] < 15 and kwargs["response_time"] > 9000:
            reencoding = True
        conclusions = vm.predict(enc_task, reencoding=reencoding)
