...
def monotonicity_no(Q, A, B, subset, superset):
    if (Q == "Some"):
        # Possible conclusions with Some:
        # No A B == All A notB
        # All A notB
        ...
        # Here B(A) must be equal to superset
        #-----------------------------------------------------------------------------
        # No B A == No A B
        # All A notB
        ...
        # Here A(A) must be equal to superset
        if (B == subset):
            return "Some not,"+A+","+superset
        elif (A == subset):
            return "Some not,"+B+","+superset
        elif (B == superset):
            return "Some not,"+A+","+subset
        elif (A == superset):
            return "Some not,"+B+","+subset
        #else:
        #    return "NVC"
        ...
        # No A B == All A notB
        # All B notA
        # Some C notB => Some C A by the upward entailing position of notB in the first and second premise
        # Here B(B) must be equal to subset oooops
        #
        # monotonicity with 2 nots seems not to work
        #
        #if (B == subset):
        #    return "Some not "+A+" "+superset
        #elif (A == subset):
...
class MonotonicityModel(ccobra.CCobraModel):
    def __init__(self, name='MonotonicityModel'):
        super(MonotonicityModel, self).__init__(name, ["syllogistic"], ["single-choice"])
    ...
    def predict(self, item, **kwargs):
        ...
        # for debugging
        #if (syllogism.encoded_task == "EI1"):
        #    print("\n",syllogism)
        #    print("\n",ccobra.syllogistic.encode_response(response, item.task), ccobra.syllogistic.decode_response(response, item.task))
         #    print("\n",response)
        #    print("\n",syllogism.logically_valid_conclusions(),ccobra.syllogistic.decode_response(syllogism.logically_valid_conclusions(), item.task))
        #    print("\n",ccobra.syllogistic.decode_response(ccobra.syllogistic.encode_response(response, item.task), item.task))
        return ccobra.syllogistic.decode_response(ccobra.syllogistic.encode_response(response, item.task), item.task)
