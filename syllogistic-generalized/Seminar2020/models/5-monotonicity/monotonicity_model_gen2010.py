...
def monotonicity_no(Q, A, B, not_subset, not_superset):
    if (Q == "Some"):
        ...
        # Here B(B) must be equal to not_subset oooops
        #
...
assert solve("No", "A", "B", "Most", "C", "A") == "Some not,C,B"
assert solve("No", "A", "B", "Few", "C", "B") == "Some not,C,A"
assert solve("No", "B", "A", "Most", "C", "A") == "Some not,C,B"
assert solve("No", "B", "A", "Few", "C", "B") == "Some not,C,A"
...
class MonotonicityModel(ccobra.CCobraModel):
    ...
    def predict(self, item, **kwargs):
        ...
        if (response1 in item.choices):
        	pred_idx = item.choices.index(response1)
