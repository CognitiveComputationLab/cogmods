...
class FastFrugalTreeClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, construction_algorithm='marginal_fan', scorer=accuracy_score, max_levels=4,
    ...
    def _get_thresholds(self, X, y):
        ...
        for i, col in enumerate(X):
            ...
            j = 0
            try:
                testvar = X[col].dtype.name == 'category'
            except:
                continue
    ...
    def _predict_all(X, cue_df):
        ...
        def prediction_func(row):
            ...
            for index, cue_row in cue_df.iterrows():
                ...
                sNew = pd.Series([outcome],index=[index])
                ret_ser = ret_ser.append(sNew, verify_integrity=True)
    ...
    def get_tree(self, idx=None, decision_view=True, top10=False):
        ...
        tres_sorted = self.all_trees.sort_values(self.scorer.__name__)
        tres_sorted.reindex(index=tres_sorted.index[::-1])
        printedind = []
        if top10:
            for index, row in tres_sorted.iterrows():
                if len(printedind) >= 10 or index[0] in printedind:
                    continue
                else:
    	            printedind.append(index[0] )
    	            print(self.all_trees.loc[index[0]])
        if decision_view:
            def exit_action(exit):
                ret_ser = pd.Series()
                #ret_ser.set_value('IF YES', '↓')
                #ret_ser.set_value('IF NO', '↓')
                sNewY = pd.Series(['↓'],index=['IF YES'])
                sNewN = pd.Series(['↓'],index=['IF NO'])
                ret_ser = ret_ser.append(sNewY, verify_integrity=True)
                ret_ser = ret_ser.append(sNewN, verify_integrity=True)
