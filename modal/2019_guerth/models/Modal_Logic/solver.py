...
import Modal_Logic.modalgraph as mg
import argparse
from enum import Enum
# import matplotlib.pyplot as plt


def does_a_follow_from_b(a, b, frames_list=[]):
	# return my_input("( " + a + " ) > ( " + b + " )", frames_list)
	return my_input("((" + a + ")>(" + b + "))", frames_list)


def my_input(formula, frames_list=[]):
	# print("Is " + formula + " always true?")
	formula = '~(' + formula + ')'
	parsed_formula = modalparser.parse_formula(formula)
	# print(parsed_formula)
	valid_solution_graphs = perform_graph_tableaux(parsed_formula, frames_list)
